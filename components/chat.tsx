...
export function Chat({ id, initialMessages }: ChatProps) {
  const [needsRecalibration, setNeedsRecalibration] = useState(true); // 🔥 always inject HER full prompt
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          previewToken: null,
          needsRecalibration, // 🔁 always true
          id: currentChatId,
      ...
      setNeedsRecalibration(recalibrationNeeded); // update for next round
    } catch (e: any) {
      console.error(e);
      append({
        id: nanoid(),
